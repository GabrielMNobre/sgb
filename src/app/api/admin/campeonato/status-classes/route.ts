...
import { getCampeonatoAtivo, getStatusClasses } from "@/services/campeonato";
...
export async function GET() {
  try {
    ...
    const status = await getStatusClasses(campeonato.id);
    return NextResponse.json(status);
  } catch (error) {
    console.error("Erro ao buscar status das classes:", error);
    return NextResponse.json(
