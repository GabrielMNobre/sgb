...
import { ArrowLeft, Plus, XCircle, CheckCircle } from "lucide-react";
import type {
  Acampamento,
...
export default function ParticipanteDetalhePage({
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      if (partRes.ok) {
        ...
        setParticipante(list.find((p: any) => p.id === participanteId) || null);
      }
      if (pagRes.ok) {
  ...
  const handleRegistrarPagamento = async (data: PagamentoFormData) => {
    await criarPagamentoAction(acampamentoId, { ...data, participanteId });
    await carregarDados();
  };
  ...
  const handleToggleAutorizacao = async () => {
    ...
    await marcarAutorizacaoAction(acampamentoId, participanteId, !participante.autorizacaoRecolhida);
    await carregarDados();
  };
  ...
  if (!participante) return <div className="text-center py-12"><p className="text-gray-500">Participante não encontrado</p></div>;
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => router.push(`/admin/financeiro/acampamentos/${acampamentoId}/participantes`)}>
            <ArrowLeft className="w-4 h-4" />
          </Button>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{participante.membro?.nome || "—"}</h1>
              <Badge variant={participante.status === "inscrito" ? "success" : "error"}>
        ...
        {!isFinalizado && !isCancelado && (
          <div className="flex gap-2">
            <Button variant="primary" size="sm" onClick={() => setPagamentoModalOpen(true)}>
              <Plus className="w-4 h-4 mr-1" /> Registrar Pagamento
            </Button>
            <Button size="sm" className="bg-red-600 hover:bg-red-700 text-white" onClick={() => setCancelarModalOpen(true)}>
              <XCircle className="w-4 h-4 mr-1" /> Cancelar Inscrição
            </Button>
          </div>
        )}
      </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{participante.membro?.tipo ? participante.membro.tipo.charAt(0).toUpperCase() + participante.membro.tipo.slice(1) : "—"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Unidade</span>
                <span className="font-medium">{participante.membro?.unidade?.nome || "—"}</span>
              </div>
              {participante.isento && (
                <div className="flex justify-between">
                  <span className="text-gray-600">Isento</span>
                  <Badge variant="secondary">{participante.motivoIsencao || "Sim"}</Badge>
                </div>
              )}
              {isMenor && (
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant={participante.autorizacaoRecolhida ? "success" : "warning"}>
                      {participante.autorizacaoRecolhida ? "Recolhida" : "Pendente"}
                    </Badge>
                    {!isFinalizado && !isCancelado && (
                      <Button variant="ghost" size="sm" onClick={handleToggleAutorizacao}>
              ...
              {isCancelado && participante.dataCancelamento && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatDate(participante.dataCancelamento)}</span>
                </div>
              )}
              {isCancelado && participante.valorDevolvido !== undefined && (
                <div className="flex justify-between">
                  <span className="text-gray-600">Valor Devolvido</span>
                  <span className="font-medium">{formatCurrency(participante.valorDevolvido)}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-gray-900">{formatCurrency(participante.valorAPagar)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Total Pago</span>
                <span className="font-bold text-green-600">{formatCurrency(participante.totalPago)}</span>
              </div>
              <hr />
              <div className="flex justify-between">
                <span className="text-gray-600 font-medium">Pendente</span>
                <span className={`font-bold ${participante.pendente > 0 ? "text-red-600" : "text-green-600"}`}>
              ...
              {participante.valorAPagar > 0 && (
                <div className="mt-2">
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div className="bg-green-500 h-2 rounded-full" style={{ width: `${Math.min(100, (participante.totalPago / participante.valorAPagar) * 100)}%` }} />
                  </div>
                  <p className="text-xs text-gray-500 mt-1">{((participante.totalPago / participante.valorAPagar) * 100).toFixed(1)}% pago</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="p-6">
          ...
          <PagamentosAcampamentoTable
            pagamentos={pagamentos.map((p) => ({
              ...
              participante: {
                id: participante.id,
                membro: participante.membro ? { id: participante.membro.id, nome: participante.membro.nome } : undefined,
              },
            }))}
      ...
      <PagamentoAcampamentoModal
        ...
        participantes={[{ id: participanteId, nome: participante.membro?.nome || "—" }]}
