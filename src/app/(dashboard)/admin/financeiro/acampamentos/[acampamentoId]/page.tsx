...
export default function AcampamentoDashboardPage({
  ...
  const autorizacoesPendentesLista = participantes.filter(
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => router.push("/admin/financeiro/acampamentos")}
          >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <Badge variant={isFinalizado ? "secondary" : "success"}>
        ...
        {!isFinalizado && (
          <div className="flex gap-2">
            <Button
              variant="secondary"
              size="sm"
              onClick={() => router.push(`/admin/financeiro/acampamentos/${acampamentoId}/editar`)}
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Card>
              <div className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600">Autorizações Pendentes</p>
                ...
                <div className="mt-2">
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{ width: `${Math.min(100, resumo.percentualArrecadado)}%` }}
                    />
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Card>
              <div className="p-6">
                ...
                <p className={`text-2xl font-bold mt-1 ${resumo.saldo >= 0 ? "text-green-600" : "text-red-600"}`}>
        ...
        <Card>
          <div className="p-6">
            ...
            <div className="flex flex-wrap gap-3">
              <Button variant="primary" size="sm" onClick={() => setParticipanteModalOpen(true)}>
              ...
              <Button variant="secondary" size="sm" onClick={() => router.push(`/admin/financeiro/acampamentos/${acampamentoId}/gastos`)}>
                <ShoppingCart className="w-4 h-4 mr-1" />
                Registrar Gasto
              </Button>
              <Link href={`/admin/financeiro/acampamentos/${acampamentoId}/participantes`}>
                <Button variant="secondary" size="sm">
                  <Users className="w-4 h-4 mr-1" />
                  Ver Participantes
                </Button>
              </Link>
              <Link href={`/admin/financeiro/acampamentos/${acampamentoId}/pagamentos`}>
                <Button variant="secondary" size="sm">
                  <DollarSign className="w-4 h-4 mr-1" />
                  Ver Pagamentos
                </Button>
              </Link>
              <Link href={`/admin/financeiro/acampamentos/${acampamentoId}/gastos`}>
                <Button variant="secondary" size="sm">
      ...
      {participantesComPendencia.length > 0 && (
        <Card>
          <div className="p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Participantes com Pendência</h2>
            <div className="space-y-3">
              {participantesComPendencia.slice(0, 10).map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-900">{p.membro?.nome || "—"}</p>
                    <p className="text-xs text-gray-500">Pago: {formatCurrency(p.totalPago)} de {formatCurrency(p.valorAPagar)}</p>
                  </div>
                  <span className="text-red-600 font-semibold">{formatCurrency(p.pendente)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {autorizacoesPendentesLista.length > 0 && (
        <Card>
          <div className="p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Autorizações Pendentes</h2>
            <div className="space-y-3">
              {autorizacoesPendentesLista.map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 bg-amber-50 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-900">{p.membro?.nome || "—"}</p>
                    <p className="text-xs text-gray-500">{p.membro?.unidade?.nome || "Sem unidade"}</p>
                  </div>
        ...
        <Card>
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900">Últimos Pagamentos</h2>
              <Link href={`/admin/financeiro/acampamentos/${acampamentoId}/pagamentos`}>
            ...
            <div className="space-y-3">
              {ultimosPagamentos.map((pag) => (
                <div key={pag.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-900">{pag.participante?.membro?.nome || "—"}</p>
                    <p className="text-xs text-gray-500">{formatDate(pag.data)}</p>
                  </div>
                  <span className="text-green-600 font-semibold">{formatCurrency(pag.valor)}</span>
                </div>
              ))}
            </div>
          </div>
