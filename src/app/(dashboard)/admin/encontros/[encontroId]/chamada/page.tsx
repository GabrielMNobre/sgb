    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">
            Chamada - {formatDate(encontro.data)}
          </h1>
          <p className="text-sm sm:text-base text-gray-500">
            Selecione uma unidade ou a diretoria para fazer a chamada
          </p>
        </div>
      </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <p className="text-sm text-amber-800">
            {encontro.status === "agendado"
              ? "Este encontro ainda não foi iniciado. Inicie o encontro para liberar a chamada."
              : "Este encontro já foi finalizado. A chamada é apenas para visualização."}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {unidades.map((u) => (
          <Link
            key={u.unidadeId}
            href={`/admin/encontros/${encontroId}/chamada/${u.unidadeId}`}
          >
