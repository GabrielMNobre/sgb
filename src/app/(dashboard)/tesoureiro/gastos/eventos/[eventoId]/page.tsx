    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div>
        <Button
          variant="ghost"
          size="sm"
          onClick={() => router.push("/tesoureiro/gastos/eventos")}
          className="mb-4"
        >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-2 sm:gap-0">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{evento.nome}</h1>
              <Badge variant={evento.ativo ? "success" : "secondary"}>
                {evento.ativo ? "Ativo" : "Inativo"}
              </Badge>
            </div>
            {evento.descricao && (
              <p className="text-sm sm:text-base text-gray-600 mt-2">{evento.descricao}</p>
            )}
            {evento.data && (
