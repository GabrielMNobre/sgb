      ...
      if (acampRes.ok) {
        ...
        if (acampData.acampamento?.eventoId) {
          const gastosRes = await fetch(
            `/api/gastos?eventoId=${acampData.acampamento.eventoId}`
          );
          if (gastosRes.ok) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card>
        <div className="p-6">
          {loading ? (
            ...
            <GastosTable
              gastos={gastos}
              onEdit={!isFinalizado ? handleEditarGasto : () => {}}
              onDelete={!isFinalizado ? handleExcluirGasto : () => {}}
            />
          )}
        </div>
      </Card>
      ...
      <GastoModal
        ...
        eventos={eventosParaModal}
        gastoInicial={gastoSelecionado}
        eventoIdPadrao={acampamento?.eventoId}
