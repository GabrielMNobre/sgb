...
export default function ParticipantesPage({
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      if (filtros.situacaoPagamento) params.set("situacaoPagamento", filtros.situacaoPagamento);
      if (filtros.autorizacao) params.set("autorizacao", filtros.autorizacao);
      if (filtros.busca) params.set("busca", filtros.busca);

      const [acampRes, partRes, membrosRes] = await Promise.all([
        fetch(`/api/acampamentos/${acampamentoId}`),
        fetch(`/api/acampamentos/${acampamentoId}/participantes?${params}`),
  ...
  const handleVerParticipante = (participanteId: string) => {
    router.push(
      `/tesoureiro/acampamentos/${acampamentoId}/participantes/${participanteId}`
    );
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card>
        <div className="p-6">
          <ParticipantesAcampamentoFilters
            filtros={filtros}
